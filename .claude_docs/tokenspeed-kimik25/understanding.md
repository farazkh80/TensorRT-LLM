...
## 8. Bottom line for the leadership decision

- Kernel-level win is real (~10% on small DSV3-Lite, likely larger on K2.5 in the MTP regime).
- Customer pressure (Fireworks, Together on K2.6) is real.
- The clean path is a small Python backend class today + plan to absorb into trtllm-gen post-CTM+RTS. Avoids the binary-dependency objection. Cost: one new file (`tokenspeed_mla_attention.py`) + registry entry + CLI flag, plus the existing `tokenspeed_mla.py` wrapper. **No new runtime dependency** if we use Albert's DKG-merged decode kernel (MR 21023) directly rather than the `tokenspeed-mla` wheel.
- The spec-decode parity divergence is the gate. Until Albert weighs in, the integration is a "perf measurement bridge" rather than a default-on production path.
