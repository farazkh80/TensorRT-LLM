...
def tokenspeed_batch_decode_with_kv_cache_mla(
    *,
    ...
    # Reinterpret workspace as uint8. The spike call site passes
    # `params.workspace.view(-1, 4)` (int8 view), which TokenSpeed accepts
    # because its prologue does its own view-by-uint8 internally; do an
    # explicit safe cast here so callers that pass a bf16/fp16 workspace
    # also work.
    ws = workspace_buffer
